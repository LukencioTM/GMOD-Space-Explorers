...
local function SE_CanUseToolMode(ply, mode)
	if ( !IsValid( ply ) ) then return false end
	local allowConVar = GetConVar( "toolmode_allow_" .. mode )
	if ( allowConVar && allowConVar:GetBool() ) then return true end
	if ( ply:GetNWBool( "se_sandbox_enabled", false ) && ( game.SinglePlayer() || ply:IsListenServerHost() ) ) then return true end
...
local function CC_GMOD_Tool( ply, command, arguments )
	...
	if ( !SE_CanUseToolMode( ply, targetMode ) ) then return end
